...
function canDistortion()
   return CanUse("distortion") and GetSpellInfo("W").name ~= "leblancslidereturn"
end
function canDistortionM()
   return CanUse("mimic") and mimic == "distortion" and GetSpellInfo("R").name ~= "leblancslidereturnm"
end
...
function Run()
   if GetSpellInfo("R").name == "LeblancChaosOrbM" then
      spells["mimic"] = spells["mimicMalice"]
      mimic = "malice"
   end
   if GetSpellInfo("R").name == "LeblancSlideM" then
      spells["mimic"] = spells["mimicDistortion"]
      mimic = "distortion"
   end   
   if GetSpellInfo("R").name == "LeblancSoulShackleM" then
...
function FollowUp()
   if IsOn("harrass") then
      ...
      if canDistortion() and GetSpellInfo("Q").level == 0 then
